...
This paper instruments for air pollution by leveraging changes in wind direction to estimate the causal impact of PM 2.5 on health using a subset of a sample of 40 billion observations.
...
The study estimates costs caused by exposure to pollution.
...
This study disentangles variation in exposure using the generic machine learning approach by Chernozhukov et al. (2018).
...
The authors use health care spending as a measure for medical costs and also look at health care usage.
These measures are corrected for confounding factors. 
Subsequently, their study take the estimates and aggregate them to the country level, weighting by population per capita as the dependent variable.

Further the authors create clusters for air pollution monitors to ensure that pollution not dependent on the location of the monitor.
...
Geographic variation which is enabled by using the interaction coefficient $\beta$.
...
The specification for the remaining control variables is X, again. 
...
To measure life years lost usually one needs information on counterfactual life expectancy which is unobserved.
Previous studies use e.g. counterfactual life expectancy via population life tables.
The solution proposed in this paper is using a rich set of different health and non-health characteristics to add to existing estimates only looking at sex and age.
...
There is a substantial drop in expected life expectancy when accounting for these chronic conditions and other non-health characteristics. 
...
Firstly, the paper spends a sizable amount of time defining a suitable estimate for life years left.
The authors suggest machine learning for time to event studies, looking beyond age and sex as determinants for life expectancy  to complement existing methods and for applications in economics. 
...
Further the argument that you don't need a concise estimate for identification is confusing. 
